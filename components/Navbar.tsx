...
const Navbar: React.FC<NavbarProps> = ({ currentView, onChangeView }) => {
  return (
    <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      <nav className="pointer-events-auto glass rounded-full px-4 py-3 md:px-6 lg:px-8 flex items-center justify-between shadow-glass hover:shadow-glass-hover transition-all duration-500 w-full max-w-md lg:max-w-6xl backdrop-blur-xl bg-white/80 border border-white/60">
        ...
        >
          <div className="relative w-10 h-10 rounded-xl bg-gradient-to-br from-nature-400 to-nature-300 flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform duration-300 border border-white/20">
             <span className="text-white font-fantasy font-bold text-xl leading-none pt-0.5">C</span>
             {/* Shine effect */}
          ...
          <div className="flex flex-col justify-center">
              <span className="font-fantasy font-bold text-lg tracking-widest text-slate-800 group-hover:text-nature-400 transition-colors leading-none mb-1">CLARK CHEN</span>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`
                  relative group overflow-hidden flex items-center gap-2 px-5 py-2.5 rounded-full transition-all duration-300 border
                  ${isActive 
